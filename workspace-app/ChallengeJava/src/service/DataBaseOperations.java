...
public class DataBaseOperations {
	...
	public String insertCurso(int idAlumno, int idCurso, Date fechaInsc) {
		...
		checkCarreraAlumno(idCurso, idAlumno, fechaInsc);
	...
	public void checkCarreraAlumno(int idCurso, int idAlumno, Date fechaInsc) {
		int id = 0;
		java.sql.Date sqlFechaInsc = new java.sql.Date(fechaInsc.getTime());
		String sql = "select c.identificador "
				+ "from carrera as c "
				+ "where c.identificador = (select cu.idcarrera from curso as cu where cu.identificador = ?) ";
		try {
			PreparedStatement stmt = con.prepareStatement(sql);
			...
			sql = "select idcarrera from inscripciones_carrera where idcarrera = ? and idalumno = ?";
			stmt = con.prepareStatement(sql);
			stmt.setInt(1, id);
			stmt.setInt(2, idAlumno);
			rs = stmt.executeQuery();
			if (!rs.next()) {
				sql = "insert into inscripciones_carrera(idalumno, idcarrera, fechainscripcion) values(?, ?, ?)";
				stmt = con.prepareStatement(sql);
				stmt.setInt(1, idAlumno);
				stmt.setInt(2, id);
				stmt.setDate(3, sqlFechaInsc);
	...
	public int getCursoAlumno(int idCurso, int idAlumno) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				id = Integer.valueOf(rs.getString("idalumno").trim());	
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
